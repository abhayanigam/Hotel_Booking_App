...
const MyBookings = () => {
  ...
  return (
    <div className='px-4 md:px-16 lg:px-24 xl:px-32 my-10'>
      ...
      <div className='max-w-6xl mt-8 w-full text-gray-800'>
        ...
        {bookings.map((booking)=>(
          <div key={booking._id} className='grid grid-cols-1 md:grid-cols-[3fr_2fr_1fr] w-full border-b border-gray-300 py-6 first:border-t'>
            ...
            {/*PAYMENT */}
            <div className='flex flex-col justify-center gap-4 text-sm text-gray-700 pt-3'>
              <div className='flex items-center gap-2'>
                <div className={`h-3 w-3 rounded-full ${booking.isPaid ? "bg-green-500": "bg-red-500"}`}></div>
                <p className={`text-sm ${booking.isPaid ? "text-green-500": "text-red-500"}`}>{booking.isPaid? "Paid" : "Unpaid"}</p>
              </div>
              {!booking.isPaid && (<button className='py-1.5 px-1 mt-4 text-xs border border-gray-400 rounded-full hover:bg-gray-100 transition-all cursor-pointer'>Pay Now</button>)}
            </div>
          </div>
        ))}
      </div>
